...
17. TRIM() is for VARCHAR/text columns only — NEVER TRIM(pulocationid), TRIM(dolocationid), or TRIM(locationid). In this warehouse those zone IDs are often INTEGER (or VARCHAR); TRIM(integer) causes FUNCTION_NOT_FOUND. For top-N zone ranking (e.g. yellow pickups by zone): filter trips with pulocationid IS NOT NULL, join taxi_zones on TRY_CAST(t.pulocationid AS BIGINT) = TRY_CAST(tz.locationid AS BIGINT) or CAST both AS VARCHAR — do not use TRIM(pulocationid) <> ''. Month/year filters on gtp_tlc_data still use quoted VARCHAR literals (year = '2024', month = '11').
18. Never SUBSTRING() on TIMESTAMP or datetime columns (tpep_pickup_datetime, tpep_dropoff_datetime, created_date, closed_date) — SUBSTRING only accepts CHAR/VARCHAR and causes FUNCTION_NOT_FOUND on timestamp. Weekday vs weekend: use day_of_week(ts) where Monday=1 … Sunday=7 in Trino — weekend = day_of_week(pickup_ts) IN (6, 7), else weekday. If the column is VARCHAR, parse first: day_of_week(FROM_ISO8601_TIMESTAMP(tpep_pickup_datetime)) or TRY_CAST(... AS TIMESTAMP). If already TIMESTAMP, use day_of_week(tpep_pickup_datetime) directly. Tip percentage: 100.0 * TRY_CAST(tip_amount AS DOUBLE) / NULLIF(TRY_CAST(fare_amount AS DOUBLE), 0) with fare > 0. Manhattan TLC filters: JOIN taxi_zones ON pickup zone IDs and tz.borough = 'Manhattan' (not nyc_311-style UPPERCASE borough).
19. gtp_tlc_data: filter spurious trips — TRY_CAST(trip_distance AS DOUBLE) > 0 AND <= 50 (miles), pulocationid/dolocationid IS NOT NULL, fare_amount between 0 and 500.
20. mta_turnstile (MTA subway hourly ridership): ridership is a PRE-AGGREGATED rider count per (station, hour, payment_method, fare_class_category) row — use SUM(ridership) for volume/"busiest" questions, NEVER COUNT(*). TRY_CAST ridership/transfers AS DOUBLE before SUM/AVG. transit_timestamp: TRY_CAST(... AS TIMESTAMP) (NOT FROM_ISO8601_TIMESTAMP); HOUR()/day_of_week() for time-of-day/weekday. borough is Title Case ('Manhattan'), not UPPERCASE — do not UPPER() it. payment_method is lowercase 'omny'/'metrocard'; fare_class_category splits via LIKE 'OMNY%' / 'Metrocard%'. Native lat/lon → ST_Point(longitude, latitude); H3 columns h3_r8/9/10 for hex maps. Filter year = '2025'.
