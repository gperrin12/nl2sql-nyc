...
7. ACS demographics live in census_tract_demographics with _2018 and _2023 suffixes. Rates are not pre-computed. Measure columns are STRING: prefer leaving them VARCHAR in SELECT for display; TRY_CAST(... AS BIGINT) alone often becomes NULL (whitespace/commas/format) and looks like missing data. For math use TRY_CAST(TRIM(REGEXP_REPLACE(col, ',', '')) AS BIGINT) or DOUBLE as appropriate. Use FROM_ISO8601_TIMESTAMP() for ISO date strings in nyc_311. For 311 per-capita in a calendar year like 2024, denominator usually uses total_pop_2023 (ACS vintage 2019-2023), not the calendar year in the column name.
