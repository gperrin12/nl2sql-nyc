...
12. nyc_311 per capita by neighborhood: geocode complaints with lat/lon → tract via ST_CONTAINS; aggregate counts by tract then SUM by census_tracts.ntaname (and nta2020). Join demographics on geoid; use LOWER(complaint_type) LIKE '%noise%' for noise categories. Never INNER JOIN on borough = boroname. Exclude rows with NULL/blank lat/lon only from spatial assignment (those rows cannot get an NTA).
